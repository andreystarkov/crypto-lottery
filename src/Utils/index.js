import { clone, indexOf, reject, update, sortWith, descend, prop } from 'ramda'
...
export function pushOrReplaceIfExists (item, items) {
  let output = []
  const index = items.findIndex(f => f.ticketId === item.id)
  if (index > -1) {
    output = update(index, item, items)
  } else {
    output = [ ...items, item ]
  }
  return output
...
export const checkLotteryStatus = (lottery) => {
  const { isActive, closeLotteryBlock, currentBlock, blockForRandom, isWinTicket, startLotteryBlock } = lottery
  return (isActive && closeLotteryBlock && currentBlock && closeLotteryBlock > currentBlock) ||
         ...
         (!isActive && currentBlock && startLotteryBlock && startLotteryBlock > currentBlock)
}
...
export const sortById = sortWith([
  descend(prop('id'))
])

export const add0 = e => e.length === 1 ? '0' + e : e
export const to16x = e => e ? add0(e.toString(16)) : '00'
